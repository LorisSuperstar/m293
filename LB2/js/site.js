...
function subscribeNewsletter(form) {
    ...
    if (!email) {
        if (msgEl) msgEl.textContent = "Bitte E-Mail-Adresse eingeben.";
        return;
    }
    if (msgEl)
        msgEl.textContent =
            "Danke — Sie wurden (simuliert) für den Newsletter angemeldet.";
    // optionally store in localStorage for demo
    try {
        const subs = JSON.parse(localStorage.getItem("mshop_news")) || [];
        subs.push({ email, date: new Date().toISOString() });
        localStorage.setItem("mshop_news", JSON.stringify(subs));
    } catch (e) {}
    return false;
}
