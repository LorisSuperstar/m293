...
async function renderFeatured() {
    ...
    if (featured.length === 0) {
        container.innerHTML = '<p class="muted">Keine Produkte verfügbar.</p>';
        return;
    }
    container.innerHTML = featured
        .map(
            (p) =>
                `<article class="card"><a href="product.html?id=${p.id}">${p.image ? `<img src="${p.image}" alt="${p.title}">` : `<div class="no-image">${p.title}</div>`}<h4>${p.title}</h4><p class="price">€${p.price.toFixed(2)}</p></a></article>`,
        )
        .join("");
}
...
async function renderCategories() {
    ...
    if (cats.length === 0) {
        container.innerHTML =
            '<p class="muted">Kategorien konnten nicht geladen werden.</p>';
        return;
    }
...
async function renderProductsList() {
    ...
    function show(filter) {
        ...
        container.innerHTML = list
            .map(
                (p) =>
                    `<article class="card"><a href="product.html?id=${p.id}">${p.image ? `<img src="${p.image}" alt="${p.title}">` : `<div class="no-image">${p.title}</div>`}<h4>${p.title}</h4><p class="price">€${p.price.toFixed(2)}</p></a><button class="add" data-id="${p.id}">In den Warenkorb</button></article>`,
            )
            .join("");
